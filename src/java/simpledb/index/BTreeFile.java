 ...
 * @author Becca Taft
 */
// simple db use page-level, strict two-phase locking,
// protection against phantoms effectively comes for free if the B+ tree is implemented correctly(幻读)
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		// some code goes here
		// base case
		if (BTreePageId.categToString(pid.pgcateg()) == "LEAF") {
		...
		return findLeafPage(tid, dirtypages, next.getRightChild(), Permissions.READ_ONLY, f);
	}
	...
	public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		// link chain
		BTreeLeafPage oldRightPage = page.getRightSiblingId() == null ? null
		...
		BTreeEntry bTreeEntry = new BTreeEntry(splitPoint.getField(keyField), page.getId(), rightPage.getId());
		BTreeInternalPage parentWithEmptySlots = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), field);
		parentWithEmptySlots.insertEntry(bTreeEntry);
		dirtypages.put(parentWithEmptySlots.getId(), parentWithEmptySlots);

		// update parent id
		// 因为这个只动叶子 影响的信息很少，所以并不用update parentWithEmotySlots
		updateParentPointer(tid,dirtypages,parentWithEmptySlots.pid, page.getId());
		updateParentPointer(tid,dirtypages,parentWithEmptySlots.pid, rightPage.getId());

		// update dirty pages again
		dirtypages.put(page.getId(), page);
		dirtypages.put(rightPage.getId(), rightPage);
		dirtypages.put(parentWithEmptySlots.getId(), parentWithEmptySlots);
		if (oldRightPage != null) {
		...
		Field field1 = splitPoint.getField(keyField); // parent.keyField?
		return field1.compare(Op.GREATER_THAN, field) ? page : rightPage;
...
//				emptyPage.updateEntry(next);  //这个目前没发现错误 在 exercise 2 insert  之后在观察
            }
			count++;

		}

		splitPoint.setLeftChild(page.getId());
		splitPoint.setRightChild(emptyPage.getId());

        BTreeInternalPage parentWithEmptySlots1 = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), field);
        parentWithEmptySlots1.insertEntry(splitPoint);
        updateParentPointers(tid,dirtypages,parentWithEmptySlots1);
		...
		dirtypages.put(parentWithEmptySlots1.getId(), parentWithEmptySlots1);

		Field key = splitPoint.getKey();
		return key.compare(Op.GREATER_THAN, field) ? page : emptyPage;
	 ...
	 * @throws DbException
	 */
	// 为什么要一次steal 多个数据   而不是只steal 一个数据
	// 因为插入的时候可能数据并不均衡 因为在插入的时候并没有考虑一个page 中的 tuple or entry 到没到 max/2
	// 所以只能靠delete 的时候来调整， 尽可能保证每个page 的数据量都差不多。 这个从磁盘读的时候 ，page才能被充分利用
	public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int numTuples = page.getNumTuples();
		int sibTuples = sibling.getNumTuples();
		int targetNum = (numTuples + sibTuples) >> 1;
		if (isRightSibling) {
			Iterator<Tuple> iterator = sibling.iterator();
			while (iterator.hasNext() && numTuples < targetNum) {
				Tuple next = iterator.next();
				sibling.deleteTuple(next);
				page.insertTuple(next);
				numTuples++;
			}
			Tuple last = iterator.next();
		...
		while (tupleIterator.hasNext() && numTuples < targetNum) {
			Tuple next = tupleIterator.next();
			sibling.deleteTuple(next);
			page.insertTuple(next);
			numTuples++;
		}
		Tuple last = tupleIterator.next();
		entry.setKey(last.getField(keyField));
		parent.updateEntry(entry);
		return;
	...
	public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		int numEntries = page.getNumEntries();
		int sibEntries = leftSibling.getNumEntries();
		int targetEntries = (numEntries + sibEntries) >> 1;
		Iterator<BTreeEntry> it = leftSibling.reverseIterator();
		Iterator<BTreeEntry> pageIter = page.iterator();
		BTreeEntry left = it.next();
		BTreeEntry pageFirst = pageIter.next();
		leftSibling.deleteKeyAndRightChild(left);
		page.insertEntry(new BTreeEntry(parentEntry.getKey(),left.getRightChild(), pageFirst.getLeftChild()));
		page.insertEntry(left);
		numEntries = numEntries + 2;
		while (it.hasNext() && numEntries < targetEntries ) {
	...
	public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		int numEntries = page.getNumEntries();
		int sibEntries = rightSibling.getNumEntries();
		int targetEntries = (numEntries + sibEntries) >> 1;
		Iterator<BTreeEntry> it = rightSibling.iterator();
		Iterator<BTreeEntry> pageIter = page.reverseIterator();
		BTreeEntry right = it.next();
		BTreeEntry pageFirst = pageIter.next();
		rightSibling.deleteKeyAndLeftChild(right);
		page.insertEntry(new BTreeEntry(parentEntry.getKey(),pageFirst.getRightChild(), right.getLeftChild()));
	...
	public void mergeLeafPages(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		BTreeLeafPage page = null;
		if (rightSiblingId != null) {
			page = (BTreeLeafPage) getPage(tid, dirtypages, rightSiblingId, Permissions.READ_WRITE);
	...
	public void mergeInternalPages(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		BTreeEntry left = leftIter.next();
		rightPage.deleteKeyAndLeftChild(right);
		BTreeEntry bTreeEntry = new BTreeEntry(parentEntry.getKey(), left.getRightChild() , right.getLeftChild());
		leftPage.insertEntry(bTreeEntry);
		leftPage.insertEntry(right);
		while (rightIter.hasNext()) {
