...
//    int pageSize;
    ...
    */
    //  初步猜想是拿到tid 相关的page 的原来的 image write file
    public void rollback(TransactionId tid)
        ...
        synchronized (Database.getBufferPool()) {
            synchronized(this) {
                preAppend();
                // some code goes here
                Long aLong = tidToFirstLogRecord.get(tid.getId());
                raf.seek(aLong);
...
//                                pageSet.add(before);
    ...
    */
    ...
    public void addMap(HashMap<Long, List<Page>> map, Page page, Long tid, boolean undo) {
...
//                    raf.seek(checkpoint);
    ...
    public void print() throws IOException {
        ...
        while (true) {
            try {
                int cpType = raf.readInt();
//                System.out.println("aaaaa" + raf.getFilePointer());
                long cpTid = raf.readLong();
//                System.out.println("bbbbb" + raf.getFilePointer());
