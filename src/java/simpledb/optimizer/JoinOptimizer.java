...
public class JoinOptimizer {
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        int card = 1;
        // some code goes here
        // TODO 这里暂时先这样写  看之后需要更精确的么？
        if(joinOp == Predicate.Op.EQUALS) {
            if (t1pkey || t2pkey) {
                card = (int)(Math.min(card1, card2));
            }else {
     ...
     */
    // 这里的逻辑比较绕，写明一些文字注释一下
    // 这个函数主要就是找到一个join 的序列，本质上是动态规划 把一个大问题 实现成小问题在逐一求解
    // 比如 一个sql select * from a,b,c,d where a.c0 = b.c0 and c.c0 = d.c0 and c.c0 = a.a0
    // 那么 join = { a.c0 = b.c0 , c.c0 = d.c0 , c.c0 = a.a0} 问题就是 如何排里面的序列 让 cost 最小
    // 所以就需要每一种去尝试 也就是 { a.c0 = b.c0 , {c.c0 = d.c0 , c.c0 = a.a0} }
    //                            { c.c0 = d.c0  , {a.c0 = b.c0 , c.c0 = a.a0} }
    //                            { c.c0 = a.a0  , {a.c0 = b.c0 , c.c0 = d.c0} }
    // 那么问题就变成了 次集合 {c.c0 = d.c0 , c.c0 = a.a0} 的最佳顺序 和 a.c0 = b.c0 的顺序 无非就两种情况
    // { a.c0 = b.c0 , {c.c0 = d.c0 , c.c0 = a.a0} }  和 {  {c.c0 = d.c0 , c.c0 = a.a0} , a.c0 = b.c0 }
    // 但是 如果每次求的话都展开求  复杂度太高，而且在求解的过程中，中间变量 比如 {c.c0 = d.c0 , c.c0 = a.a0} 可能在两步中都要用到
    // 所以就没有必要算两次，所以就添加一个 planCache 来保存，在操作的过程中 只需要从cache中拿就行了
    public <T> List<LogicalJoinNode> orderJoins(
        ...
        for (int i = 1; i <= joins.size() ; i++) {
            for (Set<LogicalJoinNode> s : enumerateSubsets(joins, i)) {
                ...
                // s' = s1
                // double cost = planCache.getCost(collect); 如果 get 不到， 那么map 返回是空，和前面的double 类型 对不上
                // 这里可以优化 因为enumerate 是 三重循环，复杂度比较高，虽然能过了测试 但是还是可以优化
                // 这里可以直接遍历s  <==>(等价于)  长度为1的s的子集
                for (Set<LogicalJoinNode> s1 :
                        enumerateSubsets(new ArrayList<>(s), s.size() - 1)) {
                    CostCard plan = computeCostAndCardOfSubplan(stats, filterSelectivities,
                            s.stream().filter(a -> !s1.contains(a)).findFirst().orElse(null)
                            , s, bestPlan.cost, planCache);
        ...
        Set<LogicalJoinNode> collect = joins.stream().collect(Collectors.toSet());
        if (explain) {
            printJoins(joins,planCache,stats,filterSelectivities);
        }
        return planCache.getOrder(collect);
    }
     ...
     */
    // 这里的join cost 的评估 是最大评估 也就是说
    // 无论是 a.c0 = b.c0 or a.co > b.c0 都是按照 " = " 来处理的，也就是按照 两个表 全部join 来看的
    // 因为在实际join 的时候 会有filter
