...
//                int id = 
     ...
     */
    // 根据sql 解析出logic plan 然后在 生成physical plan
    // 先 scan 然后 根据scan iter 生成filter iter  目的是先通过 filter 条件先过滤掉一部分，然后在计算出filterselectivity
    // 供 orderjoin 使用。 然后在join 最后在进行 select list 的 aggregate 操作
